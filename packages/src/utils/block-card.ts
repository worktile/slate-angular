...
export function hasBlockCardWithNode(node: DOMNode | null) {
    return !!(
        node &&
...
export function hasBlockCard(selection: DOMSelection | null) {
...
export function getCardTargetAttribute(node: DOMNode) {
    return (
        (node.parentElement?.attributes as any)?.['card-target'] ||
        (node instanceof HTMLElement && (node.attributes as any)['card-target'])
    );
}
