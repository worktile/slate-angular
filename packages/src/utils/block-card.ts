...
export function hasBlockCardWithNode(node: DOMNode) {
  return (
    node &&
    (node.parentElement.hasAttribute("card-target") ||
      (node instanceof HTMLElement && node.hasAttribute("card-target")))
  );
}

export function hasBlockCard(selection: DOMSelection) {
  return (
    hasBlockCardWithNode(selection?.anchorNode) ||
    hasBlockCardWithNode(selection?.focusNode)
  );
}

export function getCardTargetAttribute(node: DOMNode) {
  return (
    node.parentElement.attributes["card-target"] ||
