...
export abstract class ViewContainer<T extends ViewContainerItem>
  ...
  ngAfterViewInit() {
    const differ = this.differs
      .find(this.childrenComponent)
      .create((index, item) => {
        return item;
      });
    // first diff
  ...
  getPreviousRootNode(currentIndex) {
    ...
    const previousComponent = this.childrenComponent.find(
      (item, index) => index === currentIndex - 1
    );
    let previousRootNode =
      previousComponent.rootNodes[previousComponent.rootNodes.length - 1];
    if (previousRootNode) {
